RSpec.describe User do
  ...
  describe ".decide_quantity" do
    shared_examples "@quantify_of_productが，入力値を整数化した値と等しいこと" do
      it do
        allow(ARGF).to receive(:gets).and_return correct_quantify_input
        user.decide_quantity
        expect(user.quantity_of_product).to eq correct_quantify_input.to_i
      end
    end
    context "1を入力したとき" do
      let(:correct_quantify_input) { "1\n" }
      it_behaves_like "@quantify_of_productが，入力値を整数化した値と等しいこと"
    end
    context "2~100の数値のいずれかを入力したとき" do
      let(:correct_quantify_input) { "#{rand(2..100)}\n" }
      it_behaves_like "@quantify_of_productが，入力値を整数化した値と等しいこと"
    end
    ...
    shared_examples "再入力を促すこと" do
      it  do
        allow(ARGF).to receive(:gets).and_return wrong_quantify_input, correct_quantify_input
        expect{ user.decide_quantity }.to output(prompt_re_enter_msg).to_stdout
      end
    end
    context "0を入力したとき" do
      let(:wrong_quantify_input) { "0\n" }
      let(:correct_quantify_input) { "1\n" }
      it_behaves_like "再入力を促すこと"
    end
    context "負の整数を入力したとき" do
      let(:wrong_quantify_input) { "#{rand(-100..-1)}\n" }
      let(:correct_quantify_input) { "1\n" }
      it_behaves_like "再入力を促すこと"
    end
    context "数値以外の文字列を入力したとき" do
      let(:wrong_quantify_input) { "hoge\n" }
      let(:correct_quantify_input) { "1\n" }
      it_behaves_like "再入力を促すこと"
    end
  end
